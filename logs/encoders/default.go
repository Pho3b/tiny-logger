...
func (d *DefaultEncoder) LogDebug(logger interfaces.LoggerConfigsInterface, args ...interface{}) {
	if len(args) > 0 {
		...
		_, _ = fmt.Fprintln(
			os.Stdout,
			fmt.Sprintf("%vDEBUG[%s%s]:%v %s", colors[0], dateStr, timeStr, colors[1], d.buildMsg(args...)),
		)
	}
}
...
func (d *DefaultEncoder) LogInfo(logger interfaces.LoggerConfigsInterface, args ...interface{}) {
	if len(args) > 0 {
		...
		_, _ = fmt.Fprintln(
			os.Stdout,
			fmt.Sprintf("%vINFO[%s%s]:%v %s", colors[0], dateStr, timeStr, colors[1], d.buildMsg(args...)),
		)
	}
}
...
func (d *DefaultEncoder) LogWarn(logger interfaces.LoggerConfigsInterface, args ...interface{}) {
	if len(args) > 0 {
		...
		_, _ = fmt.Fprintln(
			os.Stdout,
			fmt.Sprintf("%vWARN[%s%s]:%v %s", colors[0], dateStr, timeStr, colors[1], d.buildMsg(args...)),
		)
	}
}
...
func (d *DefaultEncoder) LogError(logger interfaces.LoggerConfigsInterface, args ...interface{}) {
	if len(args) > 0 {
		...
		_, _ = fmt.Fprintln(
			os.Stderr,
			fmt.Sprintf("%vERROR[%s%s]:%v %s", colors[0], dateStr, timeStr, colors[1], d.buildMsg(args...)),
		)
	}
}
...
func (d *DefaultEncoder) LogFatalError(logger interfaces.LoggerConfigsInterface, args ...interface{}) {
	if len(args) > 0 {
		...
		_, _ = fmt.Fprintln(
			os.Stderr,
			fmt.Sprintf("%vFATAL ERROR[%s%s]:%v %s", colors[0], dateStr, timeStr, colors[1], d.buildMsg(args...)),
		)
...
func NewDefaultEncoder() *DefaultEncoder {
	...
	encoder.encoderType = interfaces.DefaultEncoderType
